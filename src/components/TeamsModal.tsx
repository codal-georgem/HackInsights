        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-hidden">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-5xl h-[85vh] overflow-hidden rounded-3xl bg-white dark:bg-[#0f111a] border border-white/20 dark:border-brand-border shadow-2xl flex flex-col"
                >
                    {/* Header */}
                    <div className="relative h-32 sm:h-40 flex-shrink-0 overflow-hidden">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                            <motion.div
                                ...
                                className="mb-2 inline-flex items-center gap-1.5 rounded-full bg-white/20 px-3 py-1 backdrop-blur-md border border-white/30"
                            >
                                <Users size={14} className="text-white" />
                                <span className="text-xs font-bold uppercase tracking-wider text-white">Hackathon 2026</span>
                            </motion.div>
                            ...
                            <motion.h2
                                ...
                                className="text-3xl sm:text-5xl font-black text-white tracking-tight drop-shadow-lg"
                            >
                                Meet the Teams
                            </motion.h2>
                        </div>
                            ...
                            <div className="space-y-12">
                                ...
                                {sortedTeams.winners.length > 0 && (
                                    <section>
                                        <div className="flex items-center gap-3 mb-6">
                                            ...
                                            <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Champions</h3>
                                        </div>
                                ...
                                <section>
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <h3 className="text-2xl font-bold text-slate-900 dark:text-white">The Contenders</h3>
                                    </div>
...
function TeamCard({ team, isWinner }: { team: Team; isWinner?: boolean }) {
    return (
        <motion.div
            ...
            className={`group relative overflow-hidden rounded-2xl border p-5 transition-all duration-300 ${isWinner
        ...
        >
            {isWinner && (
                <div className="absolute top-0 right-0 p-4">
                    ...
                    >
                        <Medal size={32} className="text-amber-500 opacity-20" />
                    </motion.div>
                </div>
            )}
            ...
            <div className="relative z-10">
                <div className="flex items-start justify-between mb-4">
                    <div>
                        <h4 className={`text-xl font-black mb-1 group-hover:text-brand-primary transition-colors ${isWinner ? "text-slate-900 dark:text-white" : "text-slate-900 dark:text-white"
                            }`}>
                            {team.projectName}
                        </h4>
                        {team.rank && (
                            <div className={`inline-flex items-center justify-center rounded-lg h-7 px-3 text-sm font-black transition-all ${team.rank === 1 ? "bg-amber-400 text-amber-950 scale-110 shadow-lg shadow-amber-400/20" :
                    ...
                    {isWinner && <Sparkles className="text-amber-400 group-hover:animate-spin-slow animate-pulse" size={20} />}
                </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {team.members.length > 0 ? (
                            team.members.map((member, i) => (
                                <span
                                    key={i}
                                    className={`rounded-lg px-2.5 py-1 text-sm font-medium border ${isWinner
                            ...
                            <span className="text-sm italic text-slate-400">Solo Vanguard</span>
                        )}
                    </div>
                </div>
            </div>
