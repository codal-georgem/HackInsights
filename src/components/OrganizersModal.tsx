...
import { X, Trophy, Heart, Sparkles, Code2 } from "lucide-react";
import { useEffect, useRef } from "react";
import Image from "next/image";
import OrganizersFlipCard from "./OrganizersFlipCard";
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 overflow-hidden">
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-lg sm:rounded-3xl sm:bg-white sm:dark:bg-[#0f111a] sm:border sm:border-white/20 sm:dark:border-brand-border sm:shadow-2xl flex flex-col"
          >
            {/* Mobile View: Flip Card (Whole Modal Flips) */}
            <div className="block sm:hidden w-full h-auto min-h-[500px] aspect-[3/4] mx-auto">
              <OrganizersFlipCard organizers={organizers} onClose={onClose} />
            </div>
            ...
            <div className="hidden sm:flex flex-col h-full overflow-hidden">
              ...
              <div className="relative flex-1 overflow-y-auto px-4 pb-8 sm:px-8">
                ...
                <motion.div
                  ...
                  className="relative mx-auto max-w-xl text-center mb-8 mt-4"
                >
                  <div className="relative z-10 rounded-2xl bg-white dark:bg-[#1a1d2d] p-5 sm:p-6 shadow-xl border border-slate-100 dark:border-brand-border/50">
                    ...
                    <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
                      ...
                      incredible event! It was an amazing opportunity to
                      innovate, collaborate, and push our boundaries. We truly
                      appreciate the platform to build something cool and
                      impactful. Looking forward to more hackathons and creative
                  ...
                  {organizers.map((org) => (
                    ...
                    >
                      <div className="relative mb-3 h-20 w-20 sm:h-24 sm:w-24 transition-all duration-300 group-hover:-translate-y-1">
                        <div
                          className={`absolute -inset-1 rounded-full bg-gradient-to-br ${org.gradient} opacity-70 blur-md group-hover:opacity-100 transition-opacity duration-500`}
                        />
                        ...
                        <div className="absolute -bottom-1 -right-1 rounded-full bg-white dark:bg-[#1a1d2d] p-1 shadow-md">
                          <div
                            className={`h-2.5 w-2.5 rounded-full bg-gradient-to-br ${org.gradient}`}
                          />
                        </div>
                      </div>
